...
public class ConnectPopup extends JDialog implements ActionListener {
	...
	public ConnectPopup() {
		...
		connectStringField.setText(HibernateService.getConfigProp(HibernateService.CONNECT_PROPERTY_NAME));
		
		usernameField = new JTextField();
		usernameField.setBounds(20, 91, 220, 25);
		usernameField.setText(HibernateService.getConfigProp(HibernateService.USERNAME_PROPERTY_NAME));
		
		passwordField = new JPasswordField();
		passwordField.setBounds(250, 91, 230, 25);
		passwordField.setText(HibernateService.getConfigProp(HibernateService.PASSWORD_PROPERTY_NAME));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (action == "ok") {
			String connString = this.connectStringField.getText(),
				   usrnString = this.usernameField.getText(),
				   pswdString = new String(this.passwordField.getPassword());
			
			try {
				HibernateService.setHibernateConnection(connString, usrnString, pswdString);
